...
import 'package:novel_oversea/core/network/dio_utils.dart';
import 'package:novel_oversea/core/network/http_utils.dart';
import 'package:novel_oversea/core/network/intercept.dart';
...
import 'package:novel_oversea/global/launch/bean/launch_bean.dart';
import 'package:novel_oversea/global/launch/controller/launch_manager.dart';
import 'package:novel_oversea/global/launch/view/launch_permission_view.dart';
...
class LaunchController extends GetxController {
  ...
  void onInit() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if(!hasGuide()) {
        FirebaseAnalytics.instance.logEvent(name: 'app_first_open');
      }
      _loadProgress();
    });
  }
        ...
        if (hasGuide()) {
          try {
            ...
            if (launchInfo?.config != null &&
                ...
                !user.isVip) {
              gotoGuide(backHome: true, isGuide: false);
            } else {
          ...
          FirebaseAnalytics.instance.logEvent(name: 'OB_show');
        }
      } else {
        timerCount.value++;
      }
  ...
  void showCheckDialog() {
    FirebaseAnalytics.instance.logEvent(name: 'privacy_show');
    showDialog(
      context: Get.context!,
  ...
  void _handleStartData(
    ...
    setToken(token)?.then((onValue) {
      if (onValue && isRefresh) {
        ...
        ///初始化依赖启动接口数据的adjust归因
        InitializeManager.initAdjust(appid: launchInfo?.adjustAppID, fbAppID: launchInfo?.adjustFBAppID);
        if(responseData.isNotEmpty) {
            ByStorageUtils.saveString(ConstString.kStartConfig, jsonEncode(responseData));
          }
        /// 上报设备信息
...
class DeviceInfoUpload {
  ...
  static void uploadUserDeviceInfo({String? pushToken}) async {
    ...
    /// 获取推送token
    final String pushToken = ByStorageUtils.getString(ConstString.kFirebasePushToken) ?? '';

    if(pushToken.isNotEmpty) {
      params["firebase_token"] = pushToken;
    }
    
    instance.isUploaded = 1;  /// 开始上传
    HttpUtils.post(
      APIs.deviceInfo, 
      showMsgWhenFailed: false,
      params, success: (data) {
        instance.isUploaded = 2; /// 完成上传
