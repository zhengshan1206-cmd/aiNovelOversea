 ...
 * @LastEditTime: 2025-09-16 11:10:14
 * @FilePath: /novel_oversea/lib/core/cache/daily_cache_manager.dart
 * @Description: 
 */
...
class DailyManager {
  ...
  static Future<bool> shouldShowPopup(String key, {DailyManagerType type = DailyManagerType.day}) async {
    ///24小时
    ...
    if(type == DailyManagerType.day) {
      ...
      if(parts.length < 2 || parts.first != currentDate || int.parse(parts.last) <= 2) {
        return true;
      }
      else {
        return false;
  ...
  // 记录当前日期为最后弹窗日期
  static Future<void> recordPopupDate(String key, {DailyManagerType type = DailyManagerType.day}) async {
    if(type == DailyManagerType.twentyfourHous) {
      final now = DateTime.now().millisecondsSinceEpoch;
    ...
    if(type != DailyManagerType.twicePerDay) {
      ...
      if(lastDate == null || lastDate.isEmpty) {
        ...
        ByStorageUtils.saveString(key, '$currentDate,${int.parse(parts.last) + 1}');
      }
    }
  }
