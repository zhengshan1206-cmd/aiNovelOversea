...
import 'package:novel_oversea/core/cache/byhy_encrypt_utils.dart';
import 'package:novel_oversea/core/network/const_keys.dart';
import 'package:novel_oversea/core/cache/log_utils.dart';
...
class DioUtils {
  ...
  DioUtils._() {
    ...
    ///
    var isTest =
        !LogUtils.inProduction || BuildConfig.instance.environment.domain.startsWith('https://192');
    if (isTest) {
      dio.httpClientAdapter = IOHttpClientAdapter(
        createHttpClient: () {
          final client = HttpClient();
  ...
  Future request<T>(
    ...
    try {
      ...
      if (connectivityResult == ConnectivityResult.none) {
        _onError(ExceptionHandle.netError, 'Error, Please check your network', onError);
        return;
      }
      ///是否是启动接口
  ...
  void _updateHeaders({bool isLoginToken = false}) {
    ...
    _dio.options.headers[ConstKeys.kLocalCountry] = ByDeviceInfoUtils.getCountryFromLocale();
    _dio.options.headers[ConstKeys.kUserLanguage] = ByDeviceInfoUtils.getLanguageFromLocale();
    _dio.options.headers[ConstKeys.kIpCountry] = ByDeviceInfoUtils.getIpCountry();
